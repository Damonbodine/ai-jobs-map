...
- **Hero becomes honest:** midpoint-centered with the range visible ("42–68 min/day, est.") instead of a max-of-optimistic-estimates point claim. *This is Damon's single biggest product decision in the roadmap — most displayed numbers will drop.* The canonical formula in `lib/timeback.ts` changes one final time, with the existing 76-test suite updated to the new invariants.
