/**
 * Applies supabase/migrations/2026-04-06-contact-messages.sql against the
 * live database via the standard `pg` Client + DATABASE_URL pattern used by
 * scripts/seed-occupations.ts.
 *
 * Run with: npx tsx scripts/apply-contact-messages-migration.ts
 *
...
async function main() {
  ...
  try {
    console.log('Applying migration: 2026-04-06-contact-messages.sql');
    ...
    if (verify.rowCount === 0) {
      throw new Error('Verification failed: contact_messages not found.');
    }

    console.log(`\nVerified contact_messages (${verify.rowCount} columns):`);
    ...
    const count = await client.query('select count(*)::int as n from contact_messages');
